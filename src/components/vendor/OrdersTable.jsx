...
const OrdersTable = ({ orders }) => {
  ...
  const handleSaveStatus = async () => {
    try {
      ...
      // Step 2: Check if the order status is CONFIRMED
      if (selectedStatus === "CONFIRMED") {
        // Step 3: Get the product details to retrieve current stock quantity
        const productResponse = await buzzar_api.get(
          `/product/details/${selectedOrder.product}/`
        );
        const currentStockQty = productResponse.data.stock_qty;

        // Step 4: Subtract the ordered quantity from the current stock quantity
        const newStockQty = currentStockQty - selectedOrder.quantity;

        // Step 5: Update the product's stock quantity using PATCH
        await buzzar_api.patch(`/product/details/${selectedOrder.product}/`, {
